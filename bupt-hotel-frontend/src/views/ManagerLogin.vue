  ...
  <div class="login-container">
    <el-card class="login-card" shadow="hover">
      <div class="login-header">
        ...
        <span class="login-title">经理登录</span>
      </div>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="80px" class="login-form" @submit.prevent>
        <el-form-item label="经理ID" prop="username">
          <el-input v-model="form.username" placeholder="请输入经理ID" prefix-icon="el-icon-user" />
        </el-form-item>
...
const rules = {
  username: [{ required: true, message: '请输入经理ID', trigger: 'blur' }],
...
async function onLogin() {
  ...
  try {
    // 调用后端登录接口（示例路径）
    const res = await axios.post('/login/manager', {
      username: form.username,
      password: form.password,
    })
    if (res.data.status === 1) {
      authStore.loginAsAdmin("manager")
