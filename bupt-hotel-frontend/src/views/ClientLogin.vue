	...
	<div class="login-container">
		<el-card class="login-card" shadow="hover">
			<div class="login-header">
				...
				<span class="login-title">客户登录</span>
			</div>
			<el-form ref="formRef" :model="form" :rules="rules" label-width="80px" class="login-form" @submit.prevent>
				<el-form-item label="房间号" prop="room_id">
					<el-input v-model="form.room_id" placeholder="请输入房间号" prefix-icon="el-icon-user" />
				</el-form-item>
...
const formRef = ref()
const form = reactive({ room_id: '', password: '' })
const rules = {
...
async function onLogin() {
	...
	try {
		...
		if (res.data.status === 1) {
			authStore.loginAsClient(Number(form.room_id), form.password)
